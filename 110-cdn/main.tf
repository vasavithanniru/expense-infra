...
resource "aws_cloudfront_distribution" "expense" {
  origin {
    domain_name  = "${var.project_name}-${var.environment}.${var.zone_name}" #expense-dev.vasavi.online
  ...
  aliases = ["${var.project_name}-cdn.${var.zone_name}"]  #expense-cdn.vasavi.online

  # dynamic content, evaluated at last no cache
  default_cache_behavior {
    allowed_methods  = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "${var.project_name}-${var.environment}.${var.zone_name}"   #expense-dev.vasavi.online
...
module "records" {
  ...
  zone_name = var.zone_name #vasavi.online
  records = [
    {
      name    = "expense-cdn" # expense-cdn.vasavi.online
      type    = "A"
      alias   = {
